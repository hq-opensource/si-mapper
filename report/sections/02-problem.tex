...
negotiation and integration project, estimated at \$5{,}000--\$15{,}000 per building~\cite{DOE_VPP2023}.
The DOE Virtual Power Plants Liftoff report identifies this directly: ``No industry standardization
...
systems that can't easily be replicated in new areas''~\cite{DOE_VPP2023}.
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  ...
  \item \textbf{Flexibility estimation.} With topology and live telemetry available, an AI
...
\textbf{The new business model.}
The ASHRAE 223P model is also the foundation of a scalable commercial agreement.
An aggregator can offer building owners the following value proposition: ``We will create and
maintain your building's semantic model, provide you with monitoring dashboards and fault detection
analytics powered by that model, and in exchange, we receive the right to adjust specific setpoints
within agreed comfort and reliability bounds during grid events.''
The building owner receives a service (the ontology, the analytics); the aggregator receives
dispatchable capacity; the grid receives a standardized, scalable enrollment path.

This positions SI-Mapper as more than a productivity tool for HVAC engineers.
It is potentially the foundational automation step in this business model: the tool that creates the
semantic model at acceptable cost and speed, making the above value exchange economically viable for
buildings of all sizes rather than only the largest, most resource-rich facilities~\cite{DOE_VPP2025}.