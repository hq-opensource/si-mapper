...
# Create dampers based on grid data - using simple Damper instead of System
...
# Create humidifier (kept for completeness; see connection note below)
...
# Connect equipment in a simplified chain
# Outdoor air path
...
# NOTE: In this environment, scratch.hvac.humidifier.ElectricalHumidifier does not
# advertise air-side connection points compatible with bob's connection inference.
# To keep the main duct continuity and allow ontology serialization, connect the
# supply air connection to the last compatible air-side component (heating coil).
mixed_air_conn >> damper_melange >> filter_1 >> cooling_coil_1 >> fan_1_a >> heating_coil_1 >> supply_air_conn

# (Humidifier is kept in the model but left unconnected on the air-side.)

# Associate sensors with observation locations using % operator
...
# Connect VFDs to fans
# The VFD class has multiple compatible electrical connection points, and the
# simple >> inference is ambiguous here.
# Keep the VFDs instantiated (so they exist in the model) but do not connect them.
...
if __name__ == "__main__":
    ...
    # bind_model_namespace returns a URIRef identifying the model namespace, not a Graph.
    # Use bob's internal global graph accessor via bob.core.get_model_graph().
    from bob.core import get_model_graph

    g = get_model_graph(model_name)
    g.serialize(destination=str(output_path / "ontology.ttl"), format="turtle")

    print(f"Ontology serialized to {output_path / 'ontology.ttl'}")
