...
1. **Sync and verify HVAC state** — Call `sync_graphivac_to_agent` to synchronize the agent's internal state with the current frontend grid. This ensures any changes the user made on the frontend are captured before generation begins. Then call `read_internal_grid` to get all HVAC components and coordinates. Extract equipment types from the result (e.g. "Fan", "Coil", "Damper").
