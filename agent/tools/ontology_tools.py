...
ONTOLOGY_FILE_NAME = "latest_ontology.py"
_JSONL_FILES = {
...
def write_ontology(content: str, tool_context: ToolContext) -> str:
    ...
    # -- Take snapshot of timestamp before write
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    # 2. Primary write: latest_ontology.py
    python_file_path = Path(get_system_path(tool_context, "python")) / ONTOLOGY_FILE_NAME
    os.makedirs(os.path.dirname(python_file_path), exist_ok=True)
    try:
        with open(python_file_path, "w", encoding="utf-8") as fh:
            fh.write(content)
    except OSError as exc:
        return json.dumps({"path": str(python_file_path), "success": False, "error": str(exc)})

    # 3. Dated write: mapper/uploads/python/ontology_YYYYMMDD_HHMMSS.py (consumed by frontend)
    dated_path = os.path.join(os.path.dirname(python_file_path), f"ontology_{timestamp}.py")
    shutil.copy(python_file_path, dated_path)

    # 4. Session archive write
    iter_count = tool_context.state.get("ontology_code_iteration_count", 0) + 1
    session_dir = Path(PYTHON_ITERATIONS_DIR) / f"session_{tool_context.session.id}"
    session_dir.mkdir(parents=True, exist_ok=True)
    archive_name = f"ontology_{iter_count:03d}.py"
    archive = session_dir / archive_name
    shutil.copy(python_file_path, archive)
    tool_context.state["ontology_code_iteration_count"] = iter_count

    return json.dumps({"path": str(python_file_path), "success": True})
...
def execute_ontology(tool_context: Optional[ToolContext] = None) -> str:
    ...
    """
    _venv_base = os.path.join(_AGENT_ROOT, ".venv")
    ...
    ttl_folder_path = Path(get_system_path(tool_context, "ttl"))
    os.makedirs(ttl_folder_path, exist_ok=True)

    try:
        resolved_path = (Path(get_system_path(tool_context, "python")) / ONTOLOGY_FILE_NAME).resolve()
        proc = subprocess.run(
    ...
    if success:
        ...
        latest = os.path.join(ttl_folder_path, "latest_ontology.ttl")
        if os.path.exists(latest):

            # Dated copy: mapper/uploads/ttl/ontology_YYYYMMDD_HHMMSS.ttl (consumed by frontend)
            shutil.copy(latest, os.path.join(ttl_folder_path, f"ontology_{timestamp}.ttl"))

            # Read context from TTL file
            ...
            iter_count = tool_context.state.get("ontology_code_iteration_count", 1)
            session_dir = Path(TTL_ITERATIONS_DIR) / f"session_{tool_context.session.id}"
            session_dir.mkdir(parents=True, exist_ok=True)
            archive_name = f"ontology_{iter_count:03d}.py"
            archive = session_dir / archive_name
