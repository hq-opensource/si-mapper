...
| **Quick run command** | `cd agent && uv run pytest tests/test_neo4j_tool.py -x -q` |
| **Full suite command** | `cd agent && uv run pytest tests/ -x -q && cd ../mapper && npm test -- --passWithNoTests` |
...
- **After every task commit:** Run `cd agent && uv run pytest tests/test_neo4j_tool.py -x -q`
...
|---------|------|------|-------------|-----------|-------------------|-------------|--------|
| 10-01-01 | 01 | 1 | Docker/Neo4j | manual | `docker compose --profile graph up -d neo4j` | ✅ / ❌ W0 | ⬜ pending |
| 10-02-01 | 02 | 1 | Python tool | unit | `cd agent && uv run pytest tests/test_neo4j_tool.py -x -q` | ❌ W0 | ⬜ pending |
| 10-03-01 | 03 | 2 | API route | unit | `cd mapper && npm test -- --testPathPattern=graph` | ❌ W0 | ⬜ pending |
| 10-04-01 | 04 | 3 | GraphWindow | manual+build | `cd mapper && npm run build 2>&1 | grep -v warning` | ❌ W0 | ⬜ pending |
...
- [ ] `agent/tests/test_neo4j_tool.py` — unit tests for LoadTtlToNeo4jTool (mocked neo4j driver)
- [ ] `mapper/src/app/api/graph/route.test.ts` — unit tests for GET /api/graph route (mocked neo4j-driver)
