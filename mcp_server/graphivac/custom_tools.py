...
def register_custom_tools(mcp_instance: FastMCP, custom_manager: CustomManager):
    ...
    @mcp_instance.tool
    def create_room_baseboard(name: str, coord: List[int]) -> ToolResult:
        """Adds a custom room baseboard heater/unit to a room at specific coordinates."""
        result = custom_manager.create_room_baseboard(name, coord)
        return ToolResult(
    ...
    @mcp_instance.tool
    def delete_room_baseboard(name: str) -> ToolResult:
        """Deletes a room baseboard by name."""
        result = custom_manager.delete_room_baseboard(name)
        return ToolResult(
            content=[TextContent(type="text", text=result["tool_status"])],
            structured_content=result
        )
    ...
    @mcp_instance.tool
    def create_pipe_chiller(name: str, coord: List[int]) -> ToolResult:
        """Adds a custom pipe chiller unit at specific coordinates."""
        result = custom_manager.create_pipe_chiller(name, coord)
        return ToolResult(
            content=[TextContent(type="text", text=result["tool_status"])],
            structured_content=result
        )
    ...
    @mcp_instance.tool
    def delete_pipe_chiller(name: str) -> ToolResult:
        """Deletes a pipe chiller by name."""
        result = custom_manager.delete_pipe_chiller(name)
        return ToolResult(
            content=[TextContent(type="text", text=result["tool_status"])],
            structured_content=result
        )